...
interface AuthContextType {
  ...
  updateData: (
    uid: string,
    displayName_u: string,
    address_u: string,
    phoneNo_u: string,
  ) => Promise<{ success: boolean; error?: string }>;
  noticesStream: noticeObj | null;
  setnoticesStream: React.Dispatch<React.SetStateAction<noticeObj | null>>;
  fetchNotices: (
    uid: string
  ) => Promise<{ success: boolean; noticesData?: noticeObj | null }>;
}
...
export function AuthContextProvider({
  ...
  const [noticesStream, setnoticesStream] = useState<noticeObj | null>(null);
  ...
  const updateData = async (
    ...
    if(!displayName_u){
      displayName_u = user?.displayName || "";
    }
    if(!address_u){
      address_u = user?.address || "";
    }
    if(!phoneNo_u){
      phoneNo_u = user?.phoneNo || "";
    }
    try {
      await updateDoc(doc(db, "users", uid), {
        "displayName" : displayName_u,
        "address" : address_u,
        "phoneNo" : phoneNo_u,
      });
  ...
  // get Notices
  const fetchNotices = async (): Promise<{ success: boolean; noticeData?: noticeObj | null }> => {
    try {
      const noticeDoc = await getDoc(doc(db, "notices"));
      if (noticeDoc.exists()) {
        return { success: true, noticeData: noticeDoc.data() as noticeObj };
      } else {
        console.error("No notice data found");
        return { success: false };
      }
    } catch (error) {
      console.error("Error fetching notcie data:", error);
      return { success: false };
    }
  };
