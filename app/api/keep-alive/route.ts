...
import { keepAliveConfig as config } from '@/utils/config/keep-alive-config';
...
export async function GET() {
  ...
  if (config?.disableRandomStringQuery != true) {
    const querySupabaseResponse: QueryResponse = await querySupabase(await supabase)
  ...
  if (config?.allowInsertionAndDeletion == true) {
    const insertOrDeleteResults: QueryResponse = await determineAction(await supabase)
